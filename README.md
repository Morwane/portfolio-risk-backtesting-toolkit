...
The chart gallery below shows the main outputs generated by the live LSEG-backed run.  
Charts are saved under `outputs/charts/` and copied to `docs/images/` for GitHub rendering.

Shows the realised NAV path of the executed portfolio after LSEG availability checks, cleaning, and portfolio construction.
...
Shows peak-to-trough losses through time. This is useful for understanding realised downside risk, not just volatility.
...
Tracks how annualised portfolio risk evolves through time.
...
Shows how risk-adjusted performance changes across regimes.
...
Calendar-style monthly performance view, useful for spotting clusters of weak or strong periods.
...
Shows the actual executed portfolio allocation after unavailable sleeves, if any, are removed and weights are renormalised.
...
Shows which sleeves contributed most to portfolio performance and risk.
...
Shows diversification structure across sleeves and highlights crowded or highly correlated exposures.
...
```bash
Shows the daily return distribution with VaR and Expected Shortfall thresholds.
